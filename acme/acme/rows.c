...
rowinit(Row *row, Rectangle r)
{
	...
	draw(screen, r, backgroundcolor, nil, ZP);
	row->r = r;
	row->col = nil;
	row->ncol = 0;
	r1 = r;
	...
	r1.min.y = r1.max.y;
	r1.max.y += Border;
	draw(screen, r1, dividercolor, nil, ZP);
...
rowresize(Row *row, Rectangle r)
{
	...
	for(i=0; i<row->ncol; i++){
		...
		if(i > 0){
			...
			draw(screen, r2, dividercolor, nil, ZP);
			r1.min.x = r2.max.x;
		}
		colresize(c, r1);
	}
...
rowdragcol(Row *row, Column *c, int _0)
{
  ...
  Found:
	...
	r.max.x = r.min.x;
	r.max.x += Border;
	draw(screen, r, dividercolor, nil, ZP);
	r.min.x = r.max.x;
	r.max.x = c->r.max.x;
	colresize(c, r);
	colmousebut(c);
	...
	for(i=0; i<j; i++){
		...
		if(i < row->ncol){
			...
			if(Dx(r1) < 50 || Dx(r2) < 50)
				continue;
			draw(screen, Rpt(r1.min, r2.max), backgroundcolor, nil, ZP);
			colresize(c1, r1);
			colresize(c2, r2);
			r2.min.x = x-Border;
			r2.max.x = x;
